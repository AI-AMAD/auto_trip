...
//    }
    ...
    //    그록이짜준코드
    @PostMapping("/auto/plan")
    public Mono<ResponseEntity<?>> autoPlan(@RequestBody TripSummaryDto tripSummaryDto) {
        try {
            // userSetting 정보 배열에 담기
            List<String> settings = new ArrayList<>();
            ...
            if (tripSummaryDto.isTourAtt()) {
                settings.add("관광"); // "관광 명소" 대신 "관광"으로 매핑
            }

            // setting에 맞는 여행 장소 검색 및 이미지 가져오기
            return planService.naverSearch(place, settings)
                    .flatMap(results -> Mono.just(ResponseEntity.ok(results)))
